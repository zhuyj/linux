/*
 * Compile Command:
 * gcc srq_server.c -o srq_server -libverbs -lrdmacm
 * 
 * Description:
 ...
 */
int
init_resources (struct context *ctx, struct rdma_addrinfo *rai)
{
  int ret, i;
 ...
 */
void
destroy_resources (struct context *ctx)
{
  int i;
 ...
 */
int
await_completion (struct context *ctx)
{
  int ret;
  struct ibv_cq *ev_cq;
  void *ev_ctx;
 ...
 */
int
run_server (struct context *ctx, struct rdma_addrinfo *rai)
{
  int ret, i;
