		 ...
		 u32 __always_unused vf_id, u8 *msg, u16 msg_size)
{
	...
	if (msg_size > IDPF_DFLT_MBX_BUF_SIZE)
		return -EINVAL;

	adapter = pci_get_drvdata(cdev_info->pdev);

	err = idpf_send_mb_msg(adapter, VIRTCHNL2_OP_RDMA, msg_size, msg);
	if (err)
		      ...
		      u16 msg_size, u8 *recv_msg, u16 *recv_len)
{
	...
	if (!recv_msg || !recv_len || msg_size > IDPF_DFLT_MBX_BUF_SIZE)
		return -EINVAL;

	size = min_t(u16, *recv_len, IDPF_DFLT_MBX_BUF_SIZE);

	adapter->rdma_data.vc_send_sync_pending = true;

	err = idpf_idc_vc_send(cdev_info, 0, send_msg, msg_size);
	if (!err) {
		err = idpf_wait_for_event(adapter, NULL, IDPF_VC_IWARP_SEND_SYNC,
			 ...
			 struct iidc_qvlist_info *qvl_info, bool map)
{
	...
	if (map)
		vop = VIRTCHNL2_OP_CONFIG_RDMA_IRQ_MAP;
	else
		vop = VIRTCHNL2_OP_RELEASE_RDMA_IRQ_MAP;

	err = idpf_send_mb_msg(adapter, vop, size, (u8 *)qvl_info);
	if (!err)
		err = idpf_wait_for_event(adapter, NULL,
					  IDPF_VC_IWARP_IRQ_MAP_UNMAP,
					  IDPF_VC_IWARP_IRQ_MAP_UNMAP_ERR);
	if (err)
