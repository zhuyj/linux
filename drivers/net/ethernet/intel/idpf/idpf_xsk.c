...
static int idpf_xsk_pool_disable(struct idpf_vport *vport, u16 qid)
{
	...
	clear_bit(qid, vport->af_xdp_zc_qps);
 ...
 * Returns 0 on success, negative on error
 */
static int idpf_xsk_pool_enable(struct idpf_vport *vport, struct xsk_buff_pool *pool, u16 qid)
{
	int err;
	...
	err = xsk_pool_dma_map(pool, &vport->adapter->pdev->dev, IDPF_RX_DMA_ATTR);
	if (err)
		return err;

	set_bit(qid, vport->af_xdp_zc_qps);
...
int idpf_xsk_pool_setup(struct idpf_vport *vport, struct xsk_buff_pool *pool, u16 qid)
{
	...
	WRITE_ONCE(vport->xsk_changing, true);
	synchronize_rcu();
...
pool_setup_failure:
	WRITE_ONCE(vport->xsk_changing, false);
	return err;
}
...
idpf_prepare_for_xmit_zc(struct idpf_queue *xdpq)
{
	...
	if (test_bit(__IDPF_Q_FLOW_SCH_EN, xdpq->flags)) {
		 ...
		 * Therefore, we are not able to use an optimization for batch processing.
		 */
		send_budget = 1;
	} else {
...
idpf_tx_splitq_clean_zc(struct idpf_queue *xdpq, u16 compl_tag)
{
	...
	return idpf_tx_clean_zc(xdpq, ntc, frames_ready);
}
...
idpf_tx_singleq_clean_zc(struct idpf_queue *xdpq, int *cleaned)
{
	...
	cleaned_stats = idpf_tx_clean_zc(xdpq, ntc, frames_ready);
...
{
	if (unlikely(vport->state == __IDPF_VPORT_DOWN))
		return -ENETDOWN;

	if (unlikely(!idpf_xdp_is_prog_ena(vport)))
		return -ENXIO;

	if (unlikely(!test_bit(xdpq_idx - vport->xdp_txq_offset, vport->af_xdp_zc_qps)))
		return -ENXIO;
 ...
 */
static void
idpf_xsk_schedule_napi_for_xmit(struct idpf_vport *vport, struct idpf_q_vector *q_vector)
{
		       ...
		       u32 __always_unused flags)
{
	...
	rcu_read_lock();
	if (READ_ONCE(vport->xsk_changing) ||
	    unlikely(mutex_is_locked(&vport->soft_reset_lock))) {
		ret = -EBUSY;
		goto exit;
	}
...
/**
 * idpf_xsk_cleanup_xdpq - remove Tx and Completion descriptors related to the XDP queue
 ...
 */
static bool idpf_rx_buf_hw_alloc_zc(struct idpf_rx_buf *buf, struct xsk_buff_pool *xsk_pool)
{
 ...
 * Returns false if all allocations were successful, true if any fail.
 */
bool idpf_rx_singleq_buf_hw_alloc_zc_all(struct idpf_queue *rxq, u16 count)
{
 ...
 */
static int
idpf_run_xdp_zc(struct idpf_queue *rxq, struct idpf_queue *xdpq, struct xdp_buff *xdp)
{
...
idpf_rx_construct_skb_zc(struct idpf_queue *rxq, struct idpf_rx_buf *rx_buf,
			 unsigned int size)
{
