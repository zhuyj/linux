			     ...
			     u8 *msg, __le16 msglen)
{
	...
	case VIRTCHNL2_OP_RELEASE_RDMA_IRQ_MAP:
		break;
	case VIRTCHNL2_OP_CONFIG_RDMA_IRQ_MAP:
		valid_len = sizeof(struct virtchnl2_rdma_qvlist_info);
		if (msglen >= valid_len) {
			struct virtchnl2_rdma_qvlist_info *qv =
				(struct virtchnl2_rdma_qvlist_info *)msg;

			if (qv->num_vectors == 0 || qv->num_vectors >
			    VIRTCHNL2_OP_CONFIG_RDMA_IRQ_MAP_MAX) {
				err_msg_format = true;
				break;
			}
