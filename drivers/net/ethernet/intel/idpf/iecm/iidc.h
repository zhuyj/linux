...
/* FIXME: the concept of passing a API version will have to be incorporated
 * into the auxiliary drivers probe calls to check if they can communicate
 * with the core PCI driver
 */
/* This major and minor version represent IDC API version information.
 * During auxiliary driver probe, auxiliary driver checks major and minor
 * version information (via. iidc_core_dev_info:ver_info).
 * If it fails and appropriate message gets logged.
 */
#define IIDC_MAJOR_VER		9
