...
#define IRDMA_MEM_REG_MIN_REQ_LEN offsetofend(struct irdma_mem_reg_req, sq_pages)
	...
	iwmr = irdma_alloc_iwmr(region, pd, virt, req.reg_type, iwdev, &iwpbl);
	if (IS_ERR(iwmr)) {
		ib_umem_release(region);
	...
	switch (req.reg_type) {
	case IRDMA_MEMREG_TYPE_QP:
		err = irdma_reg_user_mr_type_qp(req, iwdev, iwpbl, udata, iwmr);
		if (err)
			goto error;

		break;
	case IRDMA_MEMREG_TYPE_CQ:
		err = irdma_reg_user_mr_type_cq(iwdev, iwmr, iwpbl, udata, req);
		if (err)
			goto error;
		break;
